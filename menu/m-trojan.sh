...
CEKEXPIRED () {
    ...
    Exp1=$(curl -sS https://raw.githubusercontent.com/nanotechid/givpn/izin/master/sshws | grep $MYIP | awk '{print $3}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl -sS https://raw.githubusercontent.com/nanotechid/givpn/izin/master/sshws | awk '{print $4}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
